...
class TransVCL(nn.Module):
    ...
    def __init__(self, feat_transformer_config, backbone=None, head=None):
        ...
        self.mlp_head = nn.Sequential(
            nn.Linear(
                feat_transformer_config["d_model"] * 2,
                feat_transformer_config["d_model"],
            ),
            nn.ReLU(),
    ...
    def forward(
        ...
        if global_loss:
            feat1, feat2 = self.feat_encoding(
                feat1, feat2, mask1, mask2, cls_token=True, file=targets
            )
        ...
        for idx, m in enumerate(sim_matrix):
            m = m[: mask1[idx].sum().int(), : mask2[idx].sum().int()][..., None]  # type: ignore
            if self.training:
                if targets[idx].max() < 1e-6:  # type: ignore
                    global_label.append(torch.tensor([0.0], device=m.device))
                else:
                    global_label.append(torch.tensor([1.0], device=m.device))
        ...
        if global_loss and self.training:
            weight = id_.clone()  # type: ignore
            weight[id_ < self.sum_index] = 1.0
        ...
        if self.training:
            ...
            if sum(id_ < self.sum_index) > 0:
                fpn_outs_supervised = tuple(
                    [fpn_out[id_ < self.sum_index, ...] for fpn_out in fpn_outs]
                )
                ...
                (
                    ...
                    l1_loss_supervised,
                    num_fg_supervised,
                ) = self.head(fpn_outs_supervised, targets_supervised, x_supervised)
            else:
                (
                    loss_supervised,
                    iou_loss_supervised,
            ...
            outputs = {
                ...
                + self.unsupervised_weight * loss_unsupervised
                + 0.01 * global_loss,
                "iou_loss": loss_supervised
                + self.unsupervised_weight * loss_unsupervised,
                "l1_loss": l1_loss_supervised
                + self.unsupervised_weight * l1_loss_unsupervised,
                "conf_loss": conf_loss_supervised
                + self.unsupervised_weight * conf_loss_unsupervised,
                "cls_loss": cls_loss_supervised
                + self.unsupervised_weight * cls_loss_unsupervised,
                "global_loss": global_loss,
                "num_fg": num_fg_supervised
                + self.unsupervised_weight * num_fg_unsupervised,
            }
        else:
